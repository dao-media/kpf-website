...
final class Admin {
	...
	public static function render(): void {
		...
		$selected = isset( $_GET['group'] ) // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			: 0;
		$global   = isset( $_GET['global'] ) // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			? Globals::sanitize_role( wp_unslash( $_GET['global'] ) ) // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			: '';
		if ( Globals::ROLE_NONE === $global ) {
			$global = '';
		}
		...
		$posts   = self::components( $search, $selected, $global );
		...
		echo esc_html__(
			'Build visually with Foundation blocks or import an HTML, Gutenberg markup, or pattern JSON file. Review the result in the editor canvas, then choose Synced when every use should update together or Not synced when each inserted copy should be editable on its own. Assign one published component as the site header and one as the footer from the component editor.',
			'kpf-core'
		);
		echo '</p></div>';
	 ...
	 */
	private static function components( string $search, int $group, string $global = '' ): array {
		$args = array(
			'post_type'      => 'wp_block',
		...
		if ( in_array( $global, Globals::roles(), true ) ) {
			$args['meta_query'] = array(
				array(
					'key'   => Globals::ROLE_META,
					'value' => $global,
				),
			);
		}
	...
	private static function render_group( string $label, array $posts, int $depth ): void {
		...
		foreach ( $posts as $post ) {
			...
			$role        = Globals::get_role( (int) $post->ID );
			...
			echo '<a href="' . esc_url( get_edit_post_link( $post->ID, 'raw' ) ?: '#' ) .
				...
				'</strong></a>';
			if ( Globals::ROLE_HEADER === $role ) {
				echo ' <span class="kpf-component-badge kpf-component-badge--header" style="display:inline-block;margin-left:6px;padding:1px 7px;border-radius:999px;background:#1b2a4a;color:#fff;font-size:11px;font-weight:600;vertical-align:middle;">' .
					esc_html__( 'Header', 'kpf-core' ) . '</span>';
			} elseif ( Globals::ROLE_FOOTER === $role ) {
				echo ' <span class="kpf-component-badge kpf-component-badge--footer" style="display:inline-block;margin-left:6px;padding:1px 7px;border-radius:999px;background:#334155;color:#fff;font-size:11px;font-weight:600;vertical-align:middle;">' .
					esc_html__( 'Footer', 'kpf-core' ) . '</span>';
			}
	...
	private static function global_select( string $selected ): void {
		echo '<label><span class="screen-reader-text">' .
			esc_html__( 'Filter by global role', 'kpf-core' ) .
			'</span><select name="global">';
		echo '<option value="">' . esc_html__( 'All roles', 'kpf-core' ) . '</option>';
		echo '<option value="header"' . selected( $selected, 'header', false ) . '>' .
			esc_html__( 'Globals: Header', 'kpf-core' ) . '</option>';
		echo '<option value="footer"' . selected( $selected, 'footer', false ) . '>' .
			esc_html__( 'Globals: Footer', 'kpf-core' ) . '</option>';
		echo '</select></label>';
	}
