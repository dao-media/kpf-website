<?php
/**
 * Smoke tests for Events content type.
 *
 * Run with:
...
kpf_events_assert(taxonomy_exists(EventsContentType::LIVE_TAXONOMY), 'Live events taxonomy is registered');
kpf_events_assert(taxonomy_exists(EventsContentType::PARTNER_TAXONOMY), 'Partner taxonomy is registered');
...
$clean = EventsMeta::sanitize(
	array(
		...
		'co_host_term_ids' => array( 1, '2', 0 ),
	)
);

kpf_events_assert($clean['end_date'] === '2026-08-01', 'End date cannot precede start date');
kpf_events_assert($clean['end_time'] === '', 'Invalid end time is cleared');
kpf_events_assert($clean['timezone'] === 'America/New_York', 'Invalid timezone falls back');
kpf_events_assert($clean['recurrence']['by_weekday'] === array( 'FR' ), 'Weekday list is sanitized');
kpf_events_assert(count($clean['exceptions']) === 1, 'Invalid exception dates are dropped');
kpf_events_assert(count($clean['reschedules']) === 1, 'Reschedule rows are kept when dates are valid');
kpf_events_assert($clean['co_host_term_ids'] === array( 1, 2 ), 'Co-host term IDs are normalized');

$partner = wp_insert_term('Community Partners Org', EventsContentType::PARTNER_TAXONOMY);
kpf_events_assert(! is_wp_error($partner), 'Partner co-host term can be created');
...
kpf_events_assert(! is_wp_error($event_id) && $event_id > 0, 'Event can be created with meta');
...
echo "\nAll Events smoke checks passed.\n";
